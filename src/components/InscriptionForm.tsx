...
export default function InscriptionForm({ session }: { session: Session }) {
  ...
  function validateStep(): string | null {
    ...
    if (currentStep === 2) {
      ...
      if (!videoFile) return 'Veuillez ajouter une vidéo de candidature.'
      return null
    }
    return null
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!videoFile) throw new Error('Veuillez ajouter une vidéo de candidature.')
      ...
      {currentStep === 2 && (
        <div className="space-y-5">
          ...
          <div>
            ...
            <p className="text-[#f5a623] text-xs bg-[#f5a623]/10 border border-[#f5a623]/20 rounded-xl p-3 mb-3 flex items-start gap-2">
              <span>🎥</span>
              <span>Filmez-vous <strong>en train de chanter</strong> avec votre téléphone. Vidéo récente, sans montage — on veut vous entendre chanter en vrai !</span>
            </p>
            <FileZone label="" accept="video/*" file={videoFile} onChange={setVideoFile} maxSizeMb={config.max_video_size_mb} hint={`Max ${config.max_video_size_mb} Mo — Filmez en 720p si la vidéo est trop lourde`} />
          </div>
