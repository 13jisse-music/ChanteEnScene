  ...
  return (
    <div className={`space-y-6 ${loading ? 'opacity-50 pointer-events-none' : ''}`}>
      ...
      <div className="bg-[#161228] border border-[#f5a623]/20 rounded-2xl p-6 space-y-4">
        <h2 className="font-bold text-sm text-[#f5a623]">Palmarès 2025</h2>
        <p className="text-white/40 text-xs">
          Injecte les 4 gagnants de l&apos;édition 2025 (avec photos) dans une session archivée.
          Visible sur <code className="bg-white/5 px-1.5 py-0.5 rounded">/palmares</code>.
        </p>
        <div className="flex gap-3">
          <button
            onClick={async () => {
              ...
              if ('error' in res && res.error) {
                ...
                setResult(`Palmarès 2025 créé ! Gagnants: ${res.winners?.join(', ')}`)
              }
              setLoading(false)
            }}
          ...
          >
            Injecter le palmarès 2025
          </button>
          <button
            onClick={async () => {
              if (!confirm('Supprimer le palmarès 2025 ?')) return
              setLoading(true)
              setResult(null)
              ...
              if ('error' in res && res.error) {
                ...
                setResult('Palmarès 2025 supprimé.')
              }
              setLoading(false)
            }}
            className="px-5 py-2.5 rounded-xl text-sm font-medium bg-red-500/10 border border-red-500/25 text-red-400 hover:bg-red-500/20 transition-colors"
          >
            Supprimer le palmarès 2025
          </button>
        </div>
      </div>
